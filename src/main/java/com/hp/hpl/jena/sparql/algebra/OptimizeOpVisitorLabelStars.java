...
import gr.ntua.h2rdf.dpplanner.DPSolverStars1;
...
public class OptimizeOpVisitorLabelStars extends OptimizeOpVisitorDPCaching{
	public OptimizeOpVisitorLabelStars() {
		// TODO Auto-generated constructor stub
	}
	
	public OptimizeOpVisitorLabelStars(Query query, CachingExecutor cachingExecutor, HTable table, HTable indexTable, PartitionFinder partitionFinder, boolean cacheRequests, boolean cacheResults) {
	...
	public void visit(OpBGP opBGP)
    {
		try {
	    	...
	    	System.out.println("VarGraphFull:"+varGraphFull);
			...
			while(it1.hasNext()){
				...
				if(!foundStar){
					...
					Integer i = triplesStarRev.remove(tid);
					/*if(i!=null){
						starsIDs.get(i).getSecond().clear(tid);
					}*/
				}
			}
	    	...
	    	for(Entry<Integer,BitSet> e : varGraphFull.entrySet()){
	    		...
	    		if(starsVar.get(e.getKey())!=null){//star var
	    			edges.set(starsVar.get(e.getKey()).getFirst());
	    		}
	    	...
	    	DPSolverStars1 solver = new DPSolverStars1();
