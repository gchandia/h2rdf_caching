...
public class SerialQueryProcessorThreaded {
	...
	public static int executeJoin(Path outFile, Object[] join_files, Configuration joinConf) throws NotSupportedDatatypeException {
		...
		try {
			if(isLast==1){
				out = new OutputBuffer(outFile, fs);
			}
			else{
			...
			printJoin(j);
			if(isLast==1){
				ret= ((OutputBuffer)out).size();
			}
			else{
				ret= ((FSDataOutputStream)out).size();
			}
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void initialize(Path outFile, Configuration joinConf) {
    	...
    	try {
	    	...
	    	String temp=null;
	    	nonJoinVarNames = new String[50];
	    	nonJoinCol = new String[50][20];
	    	nonJoinStartRow = new byte[50][20][];
	    	nonJoinSize=0;
	    	nonJoinSizeTab=new int[50];
	    	for (int i = 0; i < nonJoinSizeTab.length; i++) {
	        	nonJoinSizeTab[i]=0;
			}

    		System.out.println(joinConf.get("input.reduceScans")+" asdasd!!!" );
	    	for (int i = 0; i < Integer.parseInt(joinConf.get("input.reduceScans")); i++) {
				...
				if (rowid.length==rowlength) {
					...
					if((table == (byte)3) && Bytes.equals(predid, TYPE)){
						subclasses = scanSubclass(objid, subclasses);
					}
	...
	private static byte[] scanSubclass(byte[] objid, byte[] subclasses) {
		...
		try {
			...
			while((result=resultScanner.next())!=null){
				...
				while(it.hasNext()){
					...
					subclasses = scanSubclass(qq, subclasses);
	...
	public static void writeOut(Text outKey2) throws NumberFormatException, NotSupportedDatatypeException {
		...
		try {
			synchronized(out){
				out.write(Bytes.toBytes(outKey2+"\n"));
			}
