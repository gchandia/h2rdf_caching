...
public class SerialQueryProcessorBulk {
	...
	private static Scan reduceScan = null;
	private static ArrayList<byte[]> filterPref = null;
	...
	public static int executeJoin(Path outFile, Object[] join_files, Configuration joinConf) throws NotSupportedDatatypeException {
	    ...
	    for (int i = 0; i < join_files.length; i++) {
	    	...
	    	int no=Integer.parseInt(((String)join_files[i]).substring(((String)join_files[i]).lastIndexOf(':')+1));
	...
	private static void initialize(Path outFile, Configuration joinConf) {
    	...
    	try {
    		SUBCLASS = (new H2RDFNode(Node.createURI("http://www.w3.org/2000/01/rdf-schema#subClassOf"))).getHashValue();
	...
	private static void printJoin(FSDataOutputStream out, int joinid) throws NotSupportedDatatypeException {
		String key, keyVar, var, value, tpat, else_value ;
		Integer i=0, in_no, bind_no;
		Enumeration<String> varnames = joinVarNames.keys();
		//System.out.println("print");

		filterPref = new ArrayList<byte[]>();
		while(varnames.hasMoreElements()){
			...
			while(e.hasMoreElements()){
				...
				if(nonJoinSize!=0){
					...
					StringTokenizer vt = new StringTokenizer(key);
					...
					while(tokenizer1.hasMoreTokens()) {
						...
						reduceJoinKey(temp2, nonjno, vid, patvals);
		...
		try {
			...
			System.out.println(filterPref.size());
			byte[][] fp= new byte[10][9];
			fp=filterPref.toArray(fp);
			Filter f = new MultipleColumnPrefixFilter(fp);
			System.out.println("2");
			reduceScan.setFilter(f);
			System.out.println("3");
			resultScanner = table.getScanner(reduceScan);
			...
			while((re = resultScanner.next())!=null){
				...
				if(re.size()!=0){
					...
					for (int j = 0; j < v.length; j++) {
						//System.out.println(vtoString(v[j].getQualifier()));
						count++;
	...
	public static void reduceJoinKey(String temp2, int nonjno, int vid, String patvals) throws NotSupportedDatatypeException {
			...
			if(nonJoinCol[vid][jj].contains("?")){
				...
				if(nonJoinStartRow[vid][jj][0]==(byte)2){//osp
					if(patvals.equals(varname1)){
						...
						reduceJoin(pinakas, b, temp1, varname2);
					}
					else{
						//pos
						byte pinakas = (byte)3;
						reduceJoin(pinakas,temp1, b,  varname1);
					}
				}
				...
				if(nonJoinStartRow[vid][jj][0]==(byte)2){//osp
					...
					reduceJoinAllVarBulk(pinakas, b1, b2, temp1);
				}
				else if(nonJoinStartRow[vid][jj][0]==(byte)3){//pos
					byte[] b1 = new byte[totsize];
					byte[] b2 = new byte[totsize];
					...
					if(size>rowlength){//uparxoun subclasses
						...
						for (int ik = 0; ik < (size-totsize-1)/totsize; ik++) {
							...
							reduceJoinAllVarBulk(pinakas, b1, b2, b3);
						...
						//System.out.println(Bytes.toStringBinary(b1));
						reduceJoinAllVarBulk(pinakas, b3, b1, b2);
					}
					...
					reduceJoinAllVarBulk(pinakas, temp1, b1, b2);
				}
			}
		}
	}
	...
	private static void reduceJoinAllVarBulk(byte pinakas, byte[] b1,
			byte[] b2, byte[] b3) {
		if(reduceScan==null){
			byte[] startr= new byte[rowlength+2];
			byte[] stopr= new byte[rowlength+2];
			...
			stopr[stopr.length-1] =(byte)MyNewTotalOrderPartitioner.MAX_HBASE_BUCKETS;
			reduceScan = new Scan();
			reduceScan.setStartRow(startr);
			reduceScan.setStopRow(stopr);
			reduceScan.setCaching(70000);
			reduceScan.setCacheBlocks(true);
			reduceScan.addFamily(Bytes.toBytes("A"));
		}
		//System.out.println(Bytes.toStringBinary(b3));
		filterPref.add(b3.clone());
	}
	...
	private static void writeOut(Text outKey2) throws NumberFormatException, NotSupportedDatatypeException {
	...
	private static String transform(String newline, String binding, String pred) throws NumberFormatException, NotSupportedDatatypeException {
	...
	private static String vtoString(byte[] value) throws NotSupportedDatatypeException {
