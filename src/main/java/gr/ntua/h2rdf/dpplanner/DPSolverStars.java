...
public class DPSolverStars {
	...
	public DPJoinPlan solve(OptimizeOpVisitorDPCaching visitor, CachingExecutor cachingExecutor) throws IOException {
		...
		totalChecks=0;
		cacheChecks=0;
		n=visitor.numTriples+visitor.starsVar.size();
		maxCoverage=0;
		
		for(Integer v : visitor.abstractEdgeGraph.descendingKeySet()){
			BitSet b = new BitSet(n);
			b.set(v);
			//System.out.println(b+ " scan "+v);
			Pair<Integer, BitSet> e = visitor.starsIDs.get(v);
			if(e!=null){
				BitSet b1 = new BitSet();
				b1.or(e.getSecond());
				b1.andNot(visitor.skeletonTriples);
				MergeJoin mergeJoin = new MergeJoin(e.getFirst(),visitor,cachingExecutor);
				for (int i = b1.nextSetBit(0); i >= 0; i = b1.nextSetBit(i+1)) {
	    			mergeJoin.add(new IndexScan(visitor.tripleIds.get(i),visitor.bgpIds.get(i),visitor,cachingExecutor));
	    		}
				mergeJoin.setEdgeGraph(b1);
				mergeJoin.computeCost();
				dptable.put(b, mergeJoin);
			}
			else{
		...
		return dptable.get(visitor.abstractTriples);
	}
	...
	private void emitCsg(BitSet s1) throws IOException {
		...
		//check cache!!!!!
		if(s1.cardinality()>=2 || visitor.starsIDs.containsKey(s1.nextSetBit(0))){
			...
			if(val!=null)
				c=val.getCost();
			List<DPJoinPlan> res= CanonicalLabel.checkCacheStars2(s1, visitor,cachingExecutor,c);
			//CachedResults res =CanonicalLabel.checkCache2(s1, visitor,cachingExecutor,c);
			cacheChecks++;
			if(res.isEmpty()){
				...
				System.out.println("Found in cache: "+res.get(0).print());
				if(s1.cardinality()>maxCoverage)
					maxCoverage=s1.cardinality();
				res.get(0).computeCost();
				DPJoinPlan p = res.get(0);
				if(s1.cardinality()==n){//final join add order and group by
					...
					if(visitor.ordered){
						...
						CachedResult temp = ((CachedResults)res.get(0)).getResultWithOrdering(o.getOrdering());
						if(temp!=null){
							p=temp;
						}
						else{
	...
	private void extendCmpLcList(List<BitSet> l, int i, BitSet X, Integer var) throws IOException {
		//System.out.println("Extend: "+l+" i:"+i +" X: "+X);
		if(i<l.size()){
	...
	private void extendCmpLcList1(List<BitSet> l, int i, BitSet X, Integer var) throws IOException {
		BitSet S = l.remove(i);
		BitSet N = neighbor(S,X);
		//System.out.println("N: "+N);
		if(N.isEmpty()){
	...
	private void emitCsgList(List<BitSet> l, Integer var) throws IOException {
		...
		for(BitSet b : l){
			...
			else if(plan.getClass().equals(MergeJoin.class)){
				//check if can be merged
				MergeJoin mplan = (MergeJoin)plan;
				if(mplan.var == var){
					scans++;
					for(IndexScan sc : mplan.scans){
						m.add(sc);
						sm.addScan(sc);
					}
					for(CachedResult sc : mplan.resultScans){
						m.add(sc);
						sm.addScan(sc);
					}
				}
			...
			else if(plan.getClass().equals(SortMergeJoin.class)){
				//check if can be merged
				SortMergeJoin smplan = (SortMergeJoin)plan;
				if(smplan.var == var){
					for(IndexScan sc : smplan.scans){
						m.add(sc);
						sm.addScan(sc);
					}
					for(CachedResult sc :smplan.resultScans){
						m.add(sc);
						sm.addScan(sc);
					}
				}
	...
	public BitSet neighbor(BitSet S, BitSet X) {
		...
		for (int i = 0; i < n; i++) {
			if(S.get(i)){
				for(BitSet s : visitor.abstractEdgeGraph.get(i).values()){
					N.or(s);
				}
			}
		}
	...
	private HashMap<Integer, BitSet> neighborhood2(BitSet S, BitSet X) {
		...
		for (int i = 0; i < n; i++) {
			if(S.get(i)){
				for(Integer v : visitor.abstractEdgeGraph.get(i).keySet()){
					BitSet entry = map.get(v);
					if(entry==null){
						entry = new BitSet(n);
						entry.or(visitor.abstractEdgeGraph.get(i).get(v));
						entry.andNot(X);
	...
	private HashMap<Integer, BitSet> neighbor2(BitSet S, BitSet X) {
		...
		for(Integer i : map.keySet()){
			...
			valn.or(val);
			valn.or(S);
			BitSet varBit = visitor.abstractVarGraph.get(i);
			int s=0, s1=0;
